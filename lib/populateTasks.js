...
function createContainerInfo(taskTypeId, health, web) {
    var mappings = [];
    if(health)
        mappings.push(new Mesos.ContainerInfo.DockerInfo.PortMapping(0, 8080, "tcp"));
    if(web)
        mappings.push(new Mesos.ContainerInfo.DockerInfo.PortMapping(0, 80, "tcp"));
...
function createTaskInfo(taskTypeId) {
    var labels = null;
    if(process.env["TASK" + taskTypeId + "_LABELS"])
    {
        var json = JSON.parse(process.env["TASK" + taskTypeId + "_LABELS"]);
        labels = [];
        for(var key in json)
            labels.push(new Mesos.Label(key, json[key]));
    }
    ...
    var task = {
        ...
        "containerInfo": createContainerInfo(taskTypeId, process.env["TASK" + taskTypeId + "_HEALTHCHECK"] != null, labels != null), // Mesos.ContainerInfo object
        ...
        "labels": labels ? new Mesos.Labels(labels) : null
    };
    var taskEnv = {};
    if (process.env["TASK" + taskTypeId + "_ENV"]) {
        taskEnv = JSON.parse(he.decode(process.env["TASK" + taskTypeId + "_ENV"]));
    }

    var license = he.decode(process.env["TASK" + taskTypeId + "_LICENSE"]);
    var config = he.decode(process.env["TASK" + taskTypeId + "_CONFIG"]);

    task.commandInfo.environment.variables.push(new Mesos.Environment.Variable("NSERVICEBUS_LICENSE", license));
    task.commandInfo.environment.variables.push(new Mesos.Environment.Variable("PULSE_CONFIG", config));
